...
class IngredientExport implements FromView
{
    ...
    {
        $transactionTraint = $this->transactionByType($this->transactionType, TransactionStock::with(['detail', 'detail.ingredient' => fn($q) => $q->withTrashed(), 'detail.histories', 'detail.ingredient.type'])->where('type', $this->type));
